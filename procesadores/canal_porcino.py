"""Procesador de Entrada de Canales Porcino para Siesa (documento EIN).

Portado desde ``4.2D_CANAL_PORCINO.py``. Igual que la entrada de canal vacuno
pero filtrando las filas con 'total costo tat' > 0. Conserva la lógica de trama
(registros 450 y 470 versión 12).
"""
...
USER = os.getenv("SIESA_CANAL_USER", "webservices")
PASSWORD = os.getenv("SIESA_CANAL_PASSWORD", "Santacruz2026*")
...
class CanalPorcino:
    ...
    def dataframe(self):
        ...
        self.data1 = self.data1[self.data1["total costo tat"] > 0]
...
def procesar(excel_path, work_dir, empresa_id=None, fecha=None, parametros=None, datos=None):
    """Ejecuta el flujo de Entrada de Canal Porcino y devuelve el resultado."""
    if not empresa_id:
        raise ValueError("Debes seleccionar la empresa.")
    if not fecha:
        raise ValueError("Debes indicar la fecha de sacrificio (AAAAMMDD).")

    proc = CanalPorcino(excel_path, work_dir, empresa_id, fecha, parametros, datos)
